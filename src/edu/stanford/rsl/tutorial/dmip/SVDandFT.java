...
public class SVDandFT {
	...
	public static void invertSVD(SimpleMatrix A)
	{
		...
		int column = S.getCols();
		int row = S.getRows();
		
		for (int i = 0; i < row; i++) {
			for (int j = 0; j < column; j++) {
				if (S.getElement(i, j) != 0) {
					S.setElementValue(i, j, 1/S.getElement(i, j));
				}
			}
		}
		
		SimpleMatrix temp1 = SimpleOperators.multiplyMatrixProd(V, S);
		SimpleMatrix A1 = SimpleOperators.multiplyMatrixProd(temp1, U);
		...
		*/
		...
		int k = 0;
		while (v.getElement(k) != 0) {
			min = v.getElement(k);
			k++;
		}
		
		double cond = (v.max()/min);*/
	...
	public static void fourierExercise(Grid2D image)
	{
		...
		Grid2D imageTransf = imageC.getImagSubGrid(0, 0, image.getWidth(), image.getHeight());
		imageTransf.show();
	}
	...
	public static void main(String[] args) {
		...
		String filename = "/proj/i5dmip/uz43ezas/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/yu_fill.jpg";
		...
		String filenameShepp = "/home/cip/medtech2011/uz43ezas/Desktop/Shepp-Logan Phantom.tif";
